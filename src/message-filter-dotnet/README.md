...
```
dotnet new webapi --no-https --name message-filter-dotnet
...
```
dapr publish --publish-app-id message-filter-dotnet --pubsub dzpubsub --topic newdevice --data '{"id": "2", "humidity": "3", "temperature": "4", "name": "Dennis", "message": "Why I Sing the Blues"}'

{"id": "2", "subject": "Dennis", "source": "message-creator", "type": "event",  "message": "Why I Sing the Blues"}


dapr invoke --app-id message-filter-dotnet --method message/receiverequest --payload '{"id": "2", "humidity": "3", "temperature": "4", "name": "Dennis", "message": "Why I Sing the Blues"}'
...
az acr build --registry $REGISTRY_NAME --image message-filter-dotnet:latest .
```
...
```
kubectl apply -f ../../deploy/depl-message-filter-dotnet.yaml
